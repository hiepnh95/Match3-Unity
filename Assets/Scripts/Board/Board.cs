...
public class Board
{
    ...
    #region NGUYEN HONG HIEP ADD
    internal List<NormalItem.eNormalType> FindTypeCellAroundCellEmpty(Cell cell)
    {
        List<NormalItem.eNormalType> result = new List<NormalItem.eNormalType>();
        Cell celltop = FindCell(cell.BoardX, cell.BoardY + 1);
        Cell cellbottom = FindCell(cell.BoardX, cell.BoardY - 1);
        Cell cellbLeft = FindCell(cell.BoardX - 1, cell.BoardY);
        Cell cellRight = FindCell(cell.BoardX + 1, cell.BoardY);
        if (celltop != null && !celltop.IsEmpty)
        {
            NormalItem ni = celltop.Item as NormalItem;
            if (ni != null)
            {
                result.Add(ni.ItemType);
            }
        }
        if (cellbottom != null && !cellbottom.IsEmpty)
        {
            NormalItem ni = cellbottom.Item as NormalItem;
            if (ni != null)
            {
                result.Add(ni.ItemType);
            }
        }
        if (cellbLeft != null && !cellbLeft.IsEmpty)
        {
            NormalItem ni = cellbLeft.Item as NormalItem;
            if (ni != null)
            {
                result.Add(ni.ItemType);
            }
        }
        if (cellRight != null && !cellRight.IsEmpty)
        {
            NormalItem ni = cellRight.Item as NormalItem;
            if (ni != null)
            {
                result.Add(ni.ItemType);
            }
    ...
    internal NormalItem.eNormalType RandomType(Cell emptyCell)///to create a new item different from the 4 surrounding cells. At the same time, prioritize choosing the type of item that has the least amount on the board
    {
        Dictionary<NormalItem.eNormalType, int> dic = new Dictionary<NormalItem.eNormalType, int>();
        dic.Add(NormalItem.eNormalType.TYPE_ONE, 0);
        dic.Add(NormalItem.eNormalType.TYPE_TWO, 0);
        dic.Add(NormalItem.eNormalType.TYPE_THREE, 0);
        dic.Add(NormalItem.eNormalType.TYPE_FOUR, 0);
        dic.Add(NormalItem.eNormalType.TYPE_SIX, 0);
        dic.Add(NormalItem.eNormalType.TYPE_SEVEN, 0);

        List<NormalItem.eNormalType> typeCellAroundCellEmpt = FindTypeCellAroundCellEmpty(emptyCell);
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                if (!cell.IsEmpty)
                {
                    ...
                    if (ni != null)
                    {
                        NormalItem.eNormalType type = ni.ItemType;
        ...
        for (int i = 0; i < typeCellAroundCellEmpt.Count; i++)
        {
            if (dic.ContainsKey(typeCellAroundCellEmpt[i]))
            {
                dic.Remove(typeCellAroundCellEmpt[i]);
            }
        }
        ...
        {
            return sortedList[0].Key;//choosing the type of item that has the least amount on the board
        }
    }


    internal Cell FindCell(int boardX, int boardY)
    {
        if (boardX < 0 || boardY < 0 || boardX >= boardSizeX || boardY >= boardSizeY)
        {
            return null;
        }
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                if (m_cells[x, y].BoardX == boardX && m_cells[x, y].BoardY == boardY)
                {
                    return m_cells[x, y];
                }
            }
